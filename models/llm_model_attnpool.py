...
class LLMAttnPoolNetwork(nn.Module):
    def __init__(self, env, model_name="google/gemma-3-1b-it", batch_size=64):
        ...
        # Learned scalar attention over all token positions.
        # Zero init → uniform pooling at the start of training so no position is
        # arbitrarily privileged before any gradients have been seen.
        self.pool_attn = nn.Linear(hidden, 1, bias=False)
        nn.init.zeros_(self.pool_attn.weight)
        self.pool_attn.to(self.env.device)

        # Classification head — replaces the digit-token lm_head slice used in
        # LLMValueNetwork.  Xavier default init is appropriate here.
