#!/bin/bash
# Sample-efficiency sweep for gpt2-large.
# Runs 4 datasets × 6 fracs × 5 seeds = 120 jobs.
# Zero-shot (frac=0) is handled separately by run_zeroshot_all_server.sh.
#
# Memory: gpt2-large ~7 GB float32 → max_par=12 on two 49 GB GPUs (84 GB).
#
# Usage:
#   bash run_sample_efficiency_gpt2_large_parallel.sh [DM_EPOCHS]
#   bash run_nohup.sh run_sample_efficiency_gpt2_large_parallel.sh
