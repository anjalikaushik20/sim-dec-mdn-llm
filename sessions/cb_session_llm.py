...
class CB_Session(object):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(n_batches):
                ...
                for a in range(4):
                    ...
                    profit_t = torch.tensor(cost_y[nn_idx].astype("float32"),
        ...
        with torch.no_grad():
            for batch_idx in range((N + B - 1) // B):
                ...
                for a in range(4):
                    ...
                    nn_idx = nn_idx.flatten()
                    profit_t = torch.tensor(self._label_cost_y[nn_idx].astype("float32"),
                                            dtype=torch.float32, device=self.env.device)
    ...
    def dm_train_epoch_reward(self):
        ...
        for start in range(0, N, B):
            ...
            _, nn_idx = index.search(q_np, 1)
            nn_idx = nn_idx.flatten()
            selected_y = torch.tensor(cost_y[nn_idx].astype("float32"),
                                       dtype=torch.float32, device=self.env.device)
