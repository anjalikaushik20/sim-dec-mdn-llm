...
echo "Running 3 models in parallel, one per GPU (fracs sequential within each model)"

DATACO_CKPT="output/simulator/latest_run/ckpts/dataco/confused-frog-888_epoch378.pth"

# HF name : model tag : output dir name (matches models_ood structure) : GPU
MODELS=(
    "microsoft/Phi-4-mini-reasoning:phi4-mini:phi4_mini:0"
    "gpt2:gpt2:gpt2:1"
    # "gpt2-large:gpt2-large:gpt2_large:2"
)
...
# GPUs 0, 1, 2: one model each in parallel
for ENTRY in "${MODELS[@]}"; do
    IFS=":" read -r HF_NAME MODEL_TAG DIR_NAME GPU <<< "${ENTRY}"
    run_model "${HF_NAME}" "${MODEL_TAG}" "${DIR_NAME}" "${GPU}" &
done

# GPU 3: both Qwen models sequentially in a background subshell
(
    # run_model "Qwen/Qwen3-0.6B" "qwen3-0.6B" "qwen3-0.6" 3
    run_model "Qwen/Qwen3-1.7B" "qwen3-1.7B" "qwen3-1.7" 3
) &
