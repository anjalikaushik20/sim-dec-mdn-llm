# main/cb_main_llm_prompted.py
#
# Entry point for the TEXT PROMPTING decision-making path.
# The LLM uses natural language prompting over raw order features.
# No RL, no SFT, no adapter training of any kind.
...
from sessions.cb_session_llm_prompted import CB_Session



def parse_args():
    parser = argparse.ArgumentParser(description="AI4Simulation — Prompted LLM decision maker")
    ...
    # [PROMPTED MODE] DM training disabled — no optimizer needed.
    ...
    parser.add_argument("--hf_model_name", type=str, default="google/gemma-3-1b-it", help="HuggingFace model name for LLMPromptedDecisionMaker")
...
llm_model = LLMPromptedDecisionMaker(my_env, model_name=args.hf_model_name)
# ----------------------------------- Session Init -----------------------------------------------------------
info('--------------------------------Session Init------------------------------')
...
if my_env.args.train_mode == 0 or my_env.args.train_mode == 2:
    # [PROMPTED MODE] dm_train() is a no-op (returns immediately); kept for API compatibility.
    # my_session.dm_train()
