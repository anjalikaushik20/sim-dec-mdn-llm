...
from models.llm_model_prompted import LLMPromptedDecisionMaker
...
class CB_Session(object):
    ...
    def init_value_network(self, value_network=None):
        # Use provided instance or build a new LLMPromptedDecisionMaker.
        if value_network is None:
            model_name = getattr(self.env.args, "hf_model_name", None)
            batch_size = getattr(self.env.args, "batch_size", 32)
            self.value_network = LLMPromptedDecisionMaker(self.env, model_name=model_name, batch_size=batch_size)
            info(f"Initialized LLMPromptedDecisionMaker: {model_name} (batch={batch_size})")
        else:
            self.value_network = value_network
            name = getattr(self.value_network, "model_name", "unknown")
            info(f"Initialized decision maker from provided instance: {name}")

        # No optimizer — nothing is trainable in prompted mode.
    ...
    def dm_train_epoch(self):
        """
        One REINFORCE step for the decision maker.
        Returns: avg_reward, on_time_mean, profit_mean, loss
        """
        # [PROMPTED MODE] No training — LLM decides via text prompting.
        # To re-enable a training loop, remove the two lines below and restore a training implementation.
        info("[PROMPTED MODE] No training — LLM decides via text prompting.")
    ...
    def dm_train(self):
        """
        Training loop for the decision maker.
        """
        # [PROMPTED MODE] No training — LLM decides via text prompting.
        # To re-enable a training loop, remove the two lines below and restore a training implementation.
        info("[PROMPTED MODE] No training — LLM decides via text prompting.")
    ...
    def step(self):
        ...
        with torch.no_grad():
            # forward() returns one-hot [B, 4]; argmax gives action directly.
            value_network_output = self.value_network.forward(state)
