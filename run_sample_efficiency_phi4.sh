#!/bin/bash
# Sample-efficiency sweep for microsoft/Phi-4-mini-reasoning.
# Runs 4 datasets × 6 fracs × 5 seeds = 120 jobs.
# Zero-shot (frac=0) is handled separately by run_zeroshot_all_server.sh.
#
# Memory: Phi-4-mini-reasoning ~15 GB float32 → max_par=6 on two 49 GB GPUs (90 GB).
#
# Usage:
#   bash run_sample_efficiency_phi4_parallel.sh [DM_EPOCHS]
#   bash run_nohup.sh run_sample_efficiency_phi4_parallel.sh
...
GPUS=(2 3)
MODEL_TAG="phi4-mini"
HF_NAME="microsoft/Phi-4-mini-reasoning"
SEEDS=(42 0 1 2 3)
