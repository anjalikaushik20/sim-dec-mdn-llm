...
#
# Runs Serialized MLP and Frozen BERT baselines across the same 6 fracs × 3 datasets
...
#
# Total jobs: 2 baselines × 6 fracs × 3 datasets = 36
#
...
echo "=========================================="
echo " Experiment 2 — Architecture Ablation"
echo " dm_epochs=${DM_EPOCHS}  run_id=${RUN_ID}"
echo " GPU: 1 (RTX 6000 Ada, 49 GB — GPU 0 reserved for Exp 1)"
echo " Baselines: serialized_mlp, bert"
echo " Jobs: 2 × 6 fracs × 3 datasets = 36"
...
# ── Single-job launcher ──────────────────────────────────────────────────────
# Args: model_type dataset frac gpu_id log ckpt_dir
launch_job() {
    local model_type="$1" dataset="$2" frac="$3" gpu_id="$4" log="$5" ckpt_dir="$6"
    local extra_args=""
...
# Runs all 18 (frac × dataset) jobs for one baseline with MAX_PARALLEL concurrency.
run_baseline() {
    local model_type="$1" max_par="$2"
    local total_jobs=$(( ${#FRACS[@]} * ${#DATASETS[@]} ))
    ...
    for frac in "${FRACS[@]}"; do
        for dataset in "${DATASETS[@]}"; do
            ...
            local log_dir="${log_base}/${DS_LOWER}"
            local ckpt_dir="${BASE_OUT_DIR}/${dataset}/ckpts/frac${frac}/${model_type}"
            ...
            echo "  [${job_num}/${total_jobs}] GPU${gpu_id} → ${model_type}  ${dataset}  frac=${frac}  $(date '+%H:%M:%S')"

            (
                launch_job "${model_type}" "${dataset}" "${frac}" "${gpu_id}" "${log}" "${ckpt_dir}"
                local rc=$?
                if [ "${rc}" -ne 0 ]; then
                    echo "  [FAIL rc=${rc}] ${model_type} ${dataset} frac=${frac} GPU${gpu_id}" >&2
                fi
                sem_post
            ) &
            pids+=($!)
