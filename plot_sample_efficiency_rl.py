"""
Plot sample efficiency curves for the RL baseline from a single run directory.

Usage:
    python3 plot_sample_efficiency_rl.py <run_dir>

Example:
    python3 plot_sample_efficiency_rl.py /data/akaush39/sim-to-dec/output/latest_output/sample_efficiency/rl_baseline/20260520_182054

Expects log files named {dataset}_frac{frac}.log inside <run_dir>.
Saves plots as sample_efficiency_rl_*.png in the same directory.
"""
...
def parse_log(path):
    """Extract best_profit, best_on_time, best_dm_accuracy from an RL log file.
    Returns dict with those keys, or None if the file is incomplete.
    Metrics are logged as 'key value' (space-separated, no = sign).
    """
    metrics = {}
    patterns = {
        "best_profit":       re.compile(r"best_profit\s+([\d.]+)"),
        "best_on_time":      re.compile(r"best_on_time\s+([\d.]+)"),
        "best_dm_accuracy":  re.compile(r"best_dm_accuracy\s+([\d.]+)"),
    }
    with open(path) as f:
        for line in f:
...
def collect_results(run_dir):
    """
    Returns dict: dataset → sorted list of (frac, profit, on_time, dm_accuracy)
...
def plot_all(results, run_dir):
    """Combined figure: all datasets on each of the 4 metric subplots."""
    if not results:
    ...
    metric_keys   = ["best_profit", "best_on_time", "sum", "best_dm_accuracy"]
    metric_labels = ["Profit", "On-Time Ratio", "Profit + On-Time", "Accuracy (vs Dataset)"]
    ...
    for ax, key, label in zip(axes, metric_keys, metric_labels):
        for dataset, rows in sorted(results.items()):
            ...
            ax.plot(fracs, values, marker="o", label=DATASET_LABELS.get(dataset, dataset),
...
def plot_per_dataset(results, run_dir):
    ...
    for dataset, rows in sorted(results.items()):
        ...
        for ax, values, label in zip(axes,
                                     ...
                                     ["Profit", "On-Time Ratio", "Profit + On-Time", "Accuracy (vs Dataset)"]):
        ...
        fig.suptitle(f"{dset_label} — RL Baseline\n{run_id}", fontsize=12, fontweight="bold")
        plt.tight_layout()
