#!/bin/bash
# Sample-efficiency sweep for Qwen/Qwen3-4B.
# Runs 4 datasets × 6 fracs × 5 seeds = 120 jobs.
# Zero-shot (frac=0) is handled separately by run_zeroshot_all_server.sh.
#
# Memory: Qwen3-4B ~22 GB float32 → max_par=4 on two 49 GB GPUs (88 GB).
#
# Usage:
#   bash run_sample_efficiency_qwen3_4b_parallel.sh [DM_EPOCHS]
#   bash run_nohup.sh run_sample_efficiency_qwen3_4b_parallel.sh
